...
export default function RevenueCreate({ types }) {
    ...
    function handleSubmit(e) {
        e.preventDefault();
        post(route('receitas.store'), { onSuccess: () => reset() });
    }
    ...
    return (
        <AdminLayout title="Nova Receita">
            <Head title="Nova Receita" />

            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
                <div>
                    <h1 style={{ fontSize: 24, fontWeight: 700, color: '#f1f5f9', margin: 0 }}>Nova Receita</h1>
                    <p style={{ color: '#64748b', margin: '4px 0 0' }}>Registre uma nova receita no sistema</p>
                </div>
                <Link href={route('receitas.index')} style={{ padding: '8px 18px', borderRadius: 8, background: '#334155', color: '#94a3b8', textDecoration: 'none', fontSize: 14 }}>← Voltar</Link>
            </div>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div style={{ marginTop: 24, display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
                    <Link href={route('receitas.index')} style={{ padding: '10px 24px', borderRadius: 8, background: '#334155', color: '#94a3b8', textDecoration: 'none', fontSize: 14 }}>Cancelar</Link>
