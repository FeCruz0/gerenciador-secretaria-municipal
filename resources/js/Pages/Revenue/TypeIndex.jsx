...
export default function RevenueTypeIndex({ revenue_types = [], type_selected = null }) {
    const [search, setSearch] = useState('');
    const [editingId, setEditingId] = useState(type_selected?.id ?? null);

    const filtered = revenue_types.filter(t =>
    ...
    function handleCreate(e) {
        e.preventDefault();
        createForm.post(route('receita_tipos.store'), {
    ...
    function handleUpdate(e, id) {
        e.preventDefault();
        editForm.put(route('receita_tipos.update', id), {
    ...
    function handleDelete(id) {
        if (confirm('Tem certeza que deseja excluir este Tipo de Receita?')) {
            Inertia.delete(route('receita_tipos.destroy', id));
        }
    }
    ...
    return (
        <AdminLayout title="Tipos de Receita">
            <Head title="Tipos de Receita" />

            {/* Header */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
                <div>
                    <h1 style={{ fontSize: 24, fontWeight: 700, color: '#f1f5f9', margin: 0 }}>Tipos de Receita</h1>
                    <p style={{ color: '#64748b', margin: '4px 0 0', fontSize: 14 }}>
                        Configuração · {revenue_types.length} tipo{revenue_types.length !== 1 ? 's' : ''} cadastrado{revenue_types.length !== 1 ? 's' : ''}
                    </p>
                </div>
                <Link
                ...
                <div style={{ ...card }}>
                    ...
                    <div style={{ padding: 20 }}>
                        <form onSubmit={handleCreate}>
                            <div style={{ marginBottom: 16 }}>
                                ...
                                <input
                                    ...
                                    placeholder="Ex: Receita Tributária"
                                    required
                                />
                        ...
                        <div style={{ marginTop: 24, padding: '12px 16px', background: '#0f172a', borderRadius: 8, border: '1px solid #1e293b' }}>
                            ...
                            <p style={{ margin: '4px 0 0', color: '#f1f5f9', fontSize: 28, fontWeight: 700 }}>{revenue_types.length}</p>
                        </div>
                    </div>
                </div>
                ...
                <div style={{ ...card }}>
                    ...
                    {filtered.length === 0 ? (
                        <div style={{ padding: 48, textAlign: 'center', color: '#64748b' }}>
                            ...
                            <p style={{ margin: 0 }}>Nenhum tipo de receita encontrado.</p>
                        </div>
                    ) : (
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <thead>
                                <tr style={{ borderBottom: '1px solid #334155' }}>
                                    {['Tipo de Receita', 'Receitas', 'Criado em', 'Ações'].map(h => (
