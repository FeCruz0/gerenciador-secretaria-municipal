...
export default function ExpenseShow({ expense, types }) {
    ...
    function handleSubmit(e) {
        e.preventDefault();
        put(route('despesas.update', expense.id), { onSuccess: () => setEditing(false) });
    }
    ...
    const statusColor = statusColors[expense.status] || '#6b7280';

    return (
        <AdminLayout title={`Despesa: ${expense.title}`}>
            <Head title={`Despesa: ${expense.title}`} />

            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28 }}>
                <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 4 }}>
                        <h1 style={{ fontSize: 22, fontWeight: 700, color: '#f1f5f9', margin: 0 }}>{expense.title}</h1>
                        <span style={{ background: statusColor + '20', color: statusColor, padding: '3px 12px', borderRadius: 20, fontSize: 12, fontWeight: 600 }}>{expense.status}</span>
                    </div>
                    <p style={{ color: '#f87171', margin: 0, fontSize: 18, fontWeight: 700 }}>
                        R$ {expense.value ? Number(expense.value).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) : '0,00'}
                    </p>
                </div>
                <div style={{ display: 'flex', gap: 10 }}>
                    <Link href={route('despesas.index')} style={{ padding: '8px 16px', borderRadius: 8, background: '#334155', color: '#94a3b8', textDecoration: 'none', fontSize: 14 }}>← Voltar</Link>
                    {!editing && <button onClick={() => setEditing(true)} className="inline-flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white rounded-lg px-4 py-2 font-medium shadow-md shadow-indigo-600/20 transition-all duration-200 disabled:opacity-50 disabled:pointer-events-none">Editar</button>}
                </div>
            </div>
            ...
            {!editing ? (
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 20 }}>
                    <div style={{ background: '#1e293b', borderRadius: 12, padding: 24, border: '1px solid #334155' }}>
                        ...
                        <p style={{ color: '#f1f5f9', lineHeight: 1.7 }}>{expense.description || 'Sem descrição cadastrada.'}</p>
                            ...
                            <>
                                ...
                                {expense.files.map(f => (
                                    <a key={f.id} href={f.url || '#'} target="_blank" rel="noopener noreferrer"
                                        style={{ display: 'block', color: '#7dd3fc', marginBottom: 4, fontSize: 14 }}>
                                        📎 {f.name || f.file_name}
                                    </a>
                                ))}
                            </>
                    ...
                    <div style={{ background: '#1e293b', borderRadius: 12, padding: 24, border: '1px solid #334155' }}>
                        ...
                        {[
                            ['Tipo', expense.typeExpense?.title || expense.type?.title],
                            ['Competência', expense.competence],
                            ['Referência', expense.reference],
                            ['Cadastrado em', expense.created_at ? new Date(expense.created_at).toLocaleDateString('pt-BR') : null],
                        ].map(([label, value]) => (
                ...
                <form onSubmit={handleSubmit}>
                    <div style={{ background: '#1e293b', borderRadius: 12, padding: 24, border: '1px solid #334155' }}>
                        <h3 style={{ color: '#f1f5f9', fontSize: 16, marginTop: 0, marginBottom: 20 }}>Editar Despesa</h3>
