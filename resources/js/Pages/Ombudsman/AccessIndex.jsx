...
export default function AccessTypeIndex({ type_accesses = [], access_selected = null }) {
    const [search, setSearch] = useState('');
    const [editingId, setEditingId] = useState(access_selected?.id ?? null);

    const filtered = type_accesses.filter(t =>
    ...
    function handleCreate(e) {
        e.preventDefault();
        createForm.post(route('ouvidoria_acessos.store'), {
    ...
    const editForm = useForm({
        title: access_selected?.title ?? '',
        anonymous: access_selected?.anonymous ?? 0,
    });
    ...
    function handleUpdate(e, id) {
        e.preventDefault();
        editForm.put(route('ouvidoria_acessos.update', id), {
    ...
    function handleDelete(id) {
        if (confirm('Tem certeza que deseja excluir este Tipo de Acesso?')) {
            Inertia.delete(route('ouvidoria_acessos.destroy', id));
        }
    }
    ...
    return (
        <AdminLayout title="Tipos de Acesso">
            <Head title="Tipos de Acesso" />

            {/* Header */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
                <div>
                    <h1 style={{ fontSize: 24, fontWeight: 700, color: '#f1f5f9', margin: 0 }}>Tipos de Acesso</h1>
                    <p style={{ color: '#64748b', margin: '4px 0 0', fontSize: 14 }}>
                        Configuração · {type_accesses.length} tipo{type_accesses.length !== 1 ? 's' : ''} cadastrado{type_accesses.length !== 1 ? 's' : ''}
                    </p>
                </div>
                <Link
                    href={route('ombudsmen.index')}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: 24, alignItems: 'start' }}>
                ...
                <div style={{ ...card }}>
                    ...
                    <div style={{ padding: 20 }}>
                        <form onSubmit={handleCreate}>
                            <div style={{ marginBottom: 16 }}>
                                ...
                                <input
                                    ...
                                    placeholder="Ex: Identificado / Anônimo"
                                    required
                                />
                            ...
                            <div style={{ marginBottom: 16 }}>
                                <label style={label} htmlFor="new-anonymous">Identificação</label>
                                <select
                                    id="new-anonymous"
                                    style={input}
                                    value={createForm.data.anonymous}
                                    onChange={e => createForm.setData('anonymous', parseInt(e.target.value))}
                                >
                                    <option value={0}>Identificado (Requer identificação)</option>
                                    <option value={1}>Anônimo (Permite envio anônimo)</option>
                                </select>
                                {createForm.errors.anonymous && (
                                    <p style={{ color: '#f87171', fontSize: 12, marginTop: 4 }}>{createForm.errors.anonymous}</p>
                                )}
                            </div>
                        ...
                        <div style={{ marginTop: 24, padding: '12px 16px', background: '#0f172a', borderRadius: 8, border: '1px solid #1e293b' }}>
                            <p style={{ margin: 0, color: '#64748b', fontSize: 12, fontWeight: 600, textTransform: 'uppercase' }}>Total de Acessos</p>
                            <p style={{ margin: '4px 0 0', color: '#f1f5f9', fontSize: 28, fontWeight: 700 }}>{type_accesses.length}</p>
                        </div>
                    </div>
                </div>
                ...
                <div style={{ ...card }}>
                    <div style={{ padding: '16px 20px', borderBottom: '1px solid #334155', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <h2 style={{ margin: 0, fontSize: 15, fontWeight: 600, color: '#f1f5f9' }}>📋 Acessos Cadastrados</h2>
                        <input
                            type="text"
                    ...
                    {filtered.length === 0 ? (
                        <div style={{ padding: 48, textAlign: 'center', color: '#64748b' }}>
                            ...
                            <p style={{ margin: 0 }}>Nenhum tipo de acesso encontrado.</p>
                        </div>
                    ) : (
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <thead>
                                <tr style={{ borderBottom: '1px solid #334155' }}>
                                    {['Tipo de Acesso', 'Identificação', 'Manifestações', 'Criado em', 'Ações'].map(h => (
                                ...
                                {filtered.map((type, idx) => (
                                    <tr key={type.id} style={{ borderBottom: '1px solid #0f172a', background: idx % 2 === 0 ? 'transparent' : '#ffffff05' }}>
                                        <td style={{ padding: '12px 16px', color: '#f1f5f9', fontWeight: 500 }}>
                                            {editingId === type.id ? (
                                                <form onSubmit={e => handleUpdate(e, type.id)} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                                    <input
                                                        type="text"
                                                        style={input}
                                                    ...
                                                    <select
                                                        style={input}
                                                        value={editForm.data.anonymous}
                                                        onChange={e => editForm.setData('anonymous', parseInt(e.target.value))}
                                                    >
                                        ...
                                        <td style={{ padding: '12px 16px' }}>
                                            <span style={{
                                                background: type.anonymous ? '#7f1d1d20' : '#065f4620',
                                                color: type.anonymous ? '#f87171' : '#34d399',
                                                border: type.anonymous ? '1px solid #7f1d1d40' : '1px solid #065f4660',
                                                padding: '2px 8px',
                                                borderRadius: 6,
                                                fontSize: 12,
                                                fontWeight: 500
                                            }}>
                                                {type.anonymous ? 'Anônimo' : 'Identificado'}
                                            </span>
                                        </td>
                                        <td style={{ padding: '12px 16px', color: '#94a3b8' }}>
                                            <span style={{ background: '#1e3a5f', color: '#93c5fd', padding: '2px 10px', borderRadius: 20, fontSize: 12, fontWeight: 600 }}>
                                                {type.ombudsmen?.length ?? 0}
                                            </span>
                                        </td>
