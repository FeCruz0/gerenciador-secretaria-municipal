...
export default function RequestTypeIndex({ type_requests = [], request_selected = null }) {
    const [search, setSearch] = useState('');
    const [editingId, setEditingId] = useState(request_selected?.id ?? null);

    const filtered = type_requests.filter(t =>
    ...
    function handleCreate(e) {
        e.preventDefault();
        createForm.post(route('ouvidoria_requisicoes.store'), {
    ...
    function handleUpdate(e, id) {
        e.preventDefault();
        editForm.put(route('ouvidoria_requisicoes.update', id), {
    ...
    function handleDelete(id) {
        if (confirm('Tem certeza que deseja excluir este Tipo de Solicitação?')) {
            Inertia.delete(route('ouvidoria_requisicoes.destroy', id));
        }
    }
    ...
    return (
        <AdminLayout title="Tipos de Solicitação">
            <Head title="Tipos de Solicitação" />

            {/* Header */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
                <div>
                    <h1 style={{ fontSize: 24, fontWeight: 700, color: '#f1f5f9', margin: 0 }}>Tipos de Solicitação</h1>
                    <p style={{ color: '#64748b', margin: '4px 0 0', fontSize: 14 }}>
                        Configuração · {type_requests.length} tipo{type_requests.length !== 1 ? 's' : ''} cadastrado{type_requests.length !== 1 ? 's' : ''}
                    </p>
                </div>
                <Link
                ...
                <div style={{ ...card }}>
                    ...
                    <div style={{ padding: 20 }}>
                        <form onSubmit={handleCreate}>
                            <div style={{ marginBottom: 16 }}>
                                ...
                                <input
                                    ...
                                    placeholder="Ex: Reclamação / Elogio"
                                    required
                                />
                        ...
                        <div style={{ marginTop: 24, padding: '12px 16px', background: '#0f172a', borderRadius: 8, border: '1px solid #1e293b' }}>
                            ...
                            <p style={{ margin: '4px 0 0', color: '#f1f5f9', fontSize: 28, fontWeight: 700 }}>{type_requests.length}</p>
                        </div>
                    </div>
                </div>
                ...
                <div style={{ ...card }}>
                    ...
                    {filtered.length === 0 ? (
                        <div style={{ padding: 48, textAlign: 'center', color: '#64748b' }}>
                            ...
                            <p style={{ margin: 0 }}>Nenhum tipo de solicitação encontrado.</p>
                        </div>
                    ) : (
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <thead>
                                <tr style={{ borderBottom: '1px solid #334155' }}>
                                    {['Tipo de Solicitação', 'Manifestações', 'Criado em', 'Ações'].map(h => (
